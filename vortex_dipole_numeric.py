    ...
    if data_cases is None:
        if 'vortex_idx_loadpath' in paths:
            ...
            for g, d, gamma in cases:
                context = {"g": g, "d": d, "gamma": gamma}
                loadpath = paths['vortex_idx_loadpath'].format(**context)
                with open(loadpath, 'rb') as file:
                    data = dill.load(file)
                    data_cases.append(data)
        else:
...
def execute_for_g_d_gamma_cases(func, cases, paths=None, **kwargs):
    paths = paths or {}
    params = inspect.signature(func).parameters
    for g, d, gamma in cases:
        print(f'Executing case g={g}, d={d}, gamma={gamma}')
        run_args = {**kwargs}
        if "g" in params:
            run_args["g"] = g
        if "d" in params:
        ...
        for path_name, path in paths.items():
            run_paths[path_name] = path.format(**context)
        run_args['paths'] = run_paths
        func(**run_args)
...
if __name__ == '__main__':
    ...
    # execute_for_g_d_gamma_cases(itime_ev_2nd_2D, cases=cases, x_max=x_max, y_max=y_max, dx=dx, dy=dy, paths={'savepath':gs_no_vortex_path})
    # execute_for_g_d_gamma_cases(calculate_mu, cases=cases, paths={'psi_loadpath':gs_no_vortex_path,'savepath':mu_path})
    # execute_for_g_d_gamma_cases(itime_ev_2nd_vortex_dipole, cases=cases, x_max=x_max, y_max=y_max, dx=dx, dy=dy, paths={'savepath':psi_0_vortex_dp_path})
    # execute_for_g_d_gamma_cases(plot_density, cases=cases, paths={'psi_loadpath':psi_0_vortex_dp_path})
    # execute_for_g_d_gamma_cases(psi_time_ev_2nd_2D, cases=cases, dt=dt, t_max=t_max, gamma=gamma, paths={'psi_0_loadpath':psi_0_vortex_dp_path, 'mu_loadpath':mu_path, 'savepath':psi_time_ev_path})
    # execute_for_g_d_gamma_cases(extend_time_ev, cases=cases, scheme='2nd', dt=dt, t_stop=60, paths={'prev_time_ev_loadpath':psi_time_ev_path, 'mu_loadpath':mu_path, 'savepath':psi_time_ev_path})
    # execute_for_g_d_gamma_cases(make_animation, cases=cases, paths={'psi_loadpath':psi_time_ev_path, 'savepath':animation_path})
    # execute_for_g_d_gamma_cases(find_vortex, cases=cases, paths={'psi_loadpath':psi_time_ev_path, 'savepath':vortex_idx_path})
    ...
    cases_g150_no_diss = [(150, 0.8, 0), (150, 1.1, 0), (150, 1.3, 0), (150, 1.5, 0)]
    cases_lowg_no_diss = [(10, 0.8, 0), (50, 0.8, 0)]
    cases_d1p3_diss = [(500, 1.3, 0.03), (500, 1.3, 0.3)]

    plot_dict = {'Numerical vortex trajectories for g=150; d=0.8,1.1,1.3,1.5; gamma=0': (plot_vortex_trajectories, {'cases':cases_g150_no_diss, 'plot_array_shape':(2,2), 'plot_type':'line'}),
                 'Numerical vortex trajectories for g=10,50; d=0.8; gamma=0': (plot_vortex_trajectories, {'cases':cases_lowg_no_diss, 'plot_array_shape':(1,2), 'plot_type':'scatter'}),
                 'Numerical vortex trajectories for g=500; d=1.3; gamma=0.03,0.3': (plot_vortex_trajectories, {'cases':cases_d1p3_diss, 'plot_array_shape':(1,2), 'plot_type':'line', 'figsize':(10, 5)})}
