...
def itime_ev_2nd(x_max=6, y_max=6, tau_max=2, dtau=0.001, dx=0.025, dy=0.025, g=80, d=0, gamma=0):
    ...
    psi = cp.fft.ifft2(psi)
    psi /= cp.sqrt(cp.sum(cp.abs(psi) ** 2 * dx * dy))
    np.savez(f'numerical_saves/Vortex_dipole/g{g}_init_no_vortex', x2d=cp.asnumpy(x2d), y2d=cp.asnumpy(y2d),
             psi=cp.asnumpy(psi))
...
def itime_ev_2nd_vortex_dipole(x_max=6, y_max=6, tau_max=2, dtau=0.001, dx=0.025, dy=0.025, g=80, d=0.8, gamma=0):
    ...
    psi = cp.abs(psi) * vortex_phase
    psi /= cp.sqrt(cp.sum(cp.abs(psi) ** 2 * dx * dy))
    np.savez(f'numerical_saves/Vortex_dipole/g{g}_d{d:.1f}_init', x2d=cp.asnumpy(x2d), y2d=cp.asnumpy(y2d),
             psi=cp.asnumpy(psi))
    return cp.asnumpy(x2d), cp.asnumpy(y2d), cp.asnumpy(psi)
...
def psi_time_ev_4th(x2d, y2d, psi0, t, g, gamma=0, mu=0):
    ...
    for idx in range(len(t)):
        if idx % 100 == 0:
            ...
            psi_i = cp.fft.fft2(psi_i)
            print(f"Time evolution: {t[idx]:.3f} of {t[-1]:.3f}")
        for factor_num in range(3, 0, -1):
            ...
            psi_i = cp.fft.fft2(psi_i)
        psi_i *= Ma[0]
    return cp.asnumpy(t[::100]), cp.asnumpy(x2d), cp.asnumpy(y2d), cp.asnumpy(psi)
...
def psi_time_ev_from_groundstate(g, d, t_stop=20, dt=2e-4, gamma=0, scheme='4th'):
    ...
    t_ret, x2d, y2d, psi = time_ev_method(x2d, y2d, psi0, t, g, gamma=gamma, mu=mu)
    np.savez(f'numerical_saves/Vortex_dipole/g{g}_d{d:.1f}_gamma{gamma}_time_ev.npz', t=t_ret, x2d=x2d, y2d=y2d,
             psi=psi)
    return t_ret, x2d, y2d, psi


def extend_time_ev(g, d, t_stop=20, dt=2e-4, gamma=0, scheme='4th'):
    prev_data = np.load(f'numerical_saves/Vortex_dipole/g{g}_d{d:.1f}_gamma{gamma}_time_ev.npz')
    x2d, y2d, psi_prev = prev_data['x2d'], prev_data['y2d'], prev_data['psi']
    t_prev = prev_data['t']
    psi0 = psi_prev[-1]
    ...
    t_new_ret, x2d, y2d, psi_new = time_ev_method(x2d, y2d, psi0, t_for_method, g, gamma=gamma, mu=mu)

    psi = np.concatenate([psi_prev, psi_new[1:]], axis=0)
    t_combined = np.concatenate([t_prev, t_new_ret[1:]])

    np.savez(f'numerical_saves/Vortex_dipole/g{g}_d{d:.1f}_gamma{gamma}_time_ev.npz', t=t_combined, x2d=x2d, y2d=y2d,
             psi=psi)
    return t_combined, x2d, y2d, psi


def make_animation(g, d, gamma):
    print(f"Making animation for dynamics with g={g}, d={d:.1f}, gamma={gamma}")
...
def find_vortex(g, d, gamma=0, radius_threshold=4):
    print(f"Finding vortices for dynamics with g={g}, d={d:.1f}, gamma={gamma}")
...
def plot_vortex_trajectories(g, d, gamma, plot_array_shape, plot_type='scatter'):
    ...
    for idx, (g_i, d_i, gamma_i) in enumerate(zip(g, d, gamma)):
        with open(f'numerical_saves/Vortex_dipole/g{g_i}_d{d_i:.1f}_gamma{gamma_i}_vortex_idx.dill', 'rb') as file:
            data = dill.load(file)
...
def attach_x2d_y2d(g, d, x_max=6, y_max=6, dx=0.025, dy=0.025):
    num_x = int(2 * x_max / dx)
    num_y = int(2 * y_max / dy)
    x, dx = np.linspace(-x_max, x_max, num_x, endpoint=False, retstep=True)
    y, dy = np.linspace(-y_max, y_max, num_y, endpoint=False, retstep=True)
    ...
    np.savez(f'numerical_saves/Vortex_dipole/g{g}_d{d:.1f}_vortex_maps.npz', t=t, x2d=x2d, y2d=y2d,
             cw_vortex_map=cw_vortex_map, ccw_vortex_map=ccw_vortex_map)


if __name__ == '__main__':
    cases = [(500, 1.3, 0.03)]
    execute_for_g_d_gamma_cases(itime_ev_2nd, cases=cases, x_max=10, y_max=10, dx=0.05, dy=0.05)
    execute_for_g_d_gamma_cases(calculate_mu, cases=cases)
    execute_for_g_d_gamma_cases(itime_ev_2nd_vortex_dipole, cases=cases, x_max=10, y_max=10, dx=0.05, dy=0.05)
    execute_for_g_d_gamma_cases(plot_groundstate_density, cases=cases)
    execute_for_g_d_gamma_cases(psi_time_ev_from_groundstate, cases=cases, scheme='2nd', dt=5e-4, t_stop=60)
    execute_for_g_d_gamma_cases(extend_time_ev, cases=cases, scheme='2nd', dt=5e-4, t_stop=30)
