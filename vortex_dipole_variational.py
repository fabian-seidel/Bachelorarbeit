...
def derive_L_vd_gaussian():
    ...
    with open("saves/vortex_dipole/variational/L_vd_gaussian.dill", "wb") as f:
        dill.dump(L, f)
    return L

def load_L_vd_gaussian():
    with open("saves/vortex_dipole/variational/L_vd_gaussian.dill", "rb") as f:
        L = dill.load(f)
    return L

def integrate_complex_disc(integrand, z, zs, rho):
    ...
    for term in terms:
        ...
        if degree_z == degree_zs:
            n = degree_z
            term_integral = sp.pi * rho**(n + 1) / (n + 1)
            total_integral += term_integral

    return total_integral

def derive_L_vd_thomasfermi():
    ...
    L1_int = integrate_complex_disc(R1_integrand, z, zs, 2*mu)
    L2_1_int = integrate_complex_disc(L2_1_integrand, z, zs, 2*mu)
    L2_2_int = integrate_complex_disc(L2_2_integrand, z, zs, 2*mu)
    L2_3_int = integrate_complex_disc(L2_3_integrand, z, zs, 2*mu)
    ...
    with open("saves/vortex_dipole/variational/L_vd_thomasfermi.dill", "wb") as f:
        dill.dump(L, f)

    return L

def load_L_vd_thomasfermi():
    with open("saves/vortex_dipole/variational/L_vd_thomasfermi.dill", "rb") as f:
        L = dill.load(f)
    return L

def derive_R_vd_gaussian():
    g, gamma, mu = sp.symbols('g gamma mu', real=True)
    ...
    R2_integrand = sp.expand(u_dot * (z * zs / 2 - mu) * u_star)
    R2_int = sp.integrate(R2_integrand, (x, -sp.oo, sp.oo), (y, -sp.oo, sp.oo))
    R2_cc_integrand = sp.expand(u_dot_star * (z * zs / 2 - mu) * u)
    R2_cc_int = sp.integrate(R2_cc_integrand, (x, -sp.oo, sp.oo), (y, -sp.oo, sp.oo))
    ...
    R = -A2*(R1_int-R1_cc_int)/2 + A2*(R2_int-R2_cc_int) + A2**2*g*(R3_int-R3_cc_int)
    R *= sp.I * gamma
    R = R.subs(mu, 1 + g / (2 * sp.pi))
    with open("saves/vortex_dipole/variational/R_vd_gaussian.dill", "wb") as f:
        dill.dump(R, f)
    print("Finished calculating R")
    return R

def load_R_vd_gaussian():
    with open("saves/vortex_dipole/variational/R_vd_gaussian.dill", "rb") as f:
        R = dill.load(f)
    return R

def derive_R_vd_thomasfermi():
    g, mu = sp.symbols('g mu', real=True)
    ...
    R2_integrand = (z * zs / 2 - mu) * (v_dot * vs) * f2
    R2_cc_integrand = (z * zs / 2 - mu) * (vs_dot * v) * f2
    ...
    with open("saves/vortex_dipole/variational/R_vd_thomasfermi.dill", "wb") as f:
        dill.dump(R, f)

    return R

def load_R_vd_thomasfermi():
    with open("saves/vortex_dipole/variational/R_vd_thomasfermi.dill", "rb") as f:
        R = dill.load(f)
    return R
...
def solve_dynamics(x1_0, g, M_func, F_func, T = 20, rtol=1e-10, atol=1e-10):
    print(f"Solving dynamics for d={x1_0}, g={g}.")
    res = solve_ivp(z_derivatives, [0, T], [x1_0 + 0j, -x1_0 + 0j, x1_0 + 0j, -x1_0 + 0j], rtol=1e-10, atol=1e-10,
                    args=(g, M_func, F_func), method='BDF')
    print("Finished solving dynamics")
    return res.y
...
def plot_vortex_antivortex_trajectory(M_func, F_func, g, x1_0, T = 50, rtol=1e-10, atol=1e-10):
    fig, ax = plt.subplots()
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    res = solve_dynamics(x1_0, g, M_func, F_func, T=T, rtol=rtol, atol=atol)
    ax.plot(np.real(res[0,:]),np.imag(res[0,:]))
    ax.plot(np.real(res[1, :]), np.imag(res[1, :]))
    plt.show()

if __name__ == '__main__':
    # L_gaussian = load_L_vd_gaussian()
    L_thomasfermi = load_L_vd_thomasfermi()
    ...
    rtol = 1e-8
    atol = 1e-8
    plot_vortex_antivortex_trajectory(M_thomasfermi_diss, F_thomasfermi_diss, g, x1_0, T=T, rtol=rtol, atol=atol)